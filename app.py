...
def member_list():
    all_member = list(db.member.find({}, {'_id': False, 'mbti': False, 'musicURL': False,'location': False, 'hobby': False}).sort('name', 1))
...
def get_rolling(member_id):
    rolling_list = list(db.comment.find({'memberId': member_id}, {'_id': False, 'memberId': False, 'password': False}).sort('date', -1))
